...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    col = rotHue(col, iTime * 0.1); // Rotate hue over time
    col *= dotPattern(fragCoord.xy); // Apply dot pattern
...
vec3 nor = calcNormal(pos); // Normal.
        ...
        if (saveObjID < 2.5)
            nor = texBump(iChannel0, pOffs * ts, nor, 0.002 + step(saveObjID, 1.5) * 0.012);
...
float id = hash(dot(floor(pOffs + vec3(.0, .5, .5)), vec3(7, 157, 113)));
        ...
        col *= (dif + .25 + spot * .5 + vec3(.25, .3, .5) * spe) +
spe2;
        col*=
occ; // Applying occlusion.
    }
