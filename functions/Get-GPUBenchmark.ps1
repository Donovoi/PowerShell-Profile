...
'@
    ...
    try {
        ...
        Write-Logg -Message "Failed to compile the benchmark kernel. Error: $_"
        return
    }
    ...
    try {
        ...
        Write-Logg -Message "GPU Benchmark Score: $benchmarkResult"
    }
    catch {
        Write-Logg -Message "Failed to run the GPU benchmark. Error: $_"
    }
}