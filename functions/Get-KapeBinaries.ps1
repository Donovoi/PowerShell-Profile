...
Function Get-KapeBinaries {
    ...
    (
        [Parameter()]
        [string]$Dest = "$PWD\kapeDownloadedBinaries", # Where to save programs to
        [Parameter()]
        [string]$ModulePath = "$PSScriptRoot\Modules", # Path to Kape Modules directory
        [Parameter()]
        [switch]$CreateBinaryList,
        [Parameter()]
        [switch] $DownloadBinaries, # Optional switch to enable downloading of binaries after list is created       
        # Extracts list of Binary URLs and dumps to console
        [Parameter()]	
