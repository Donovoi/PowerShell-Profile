...
function Get-Everything {
  ...
  begin {
    ...
    if (-not (Test-Path $EverythingEXE -ErrorAction SilentlyContinue)) {
      ...
      # Download Everything
      $everythingclizip = Get-DownloadFile -URL 'https://www.voidtools.com/ES-1.1.0.26.zip' -OutFileDirectory $EverythingDirectory -UseAria2
      $everythingPortablezip = Get-DownloadFile -Url 'https://www.voidtools.com/Everything-1.5.0.1361a.x64.zip' -OutFileDirectory $EverythingDirectory -UseAria2
