...
function prompt {
  ...
  if (-not (Get-Command Write-VcsStatus -ErrorAction SilentlyContinue)) {
    if ($isFS -and (Test-Git (Convert-Path .)) -and (Get-Command git)) {
      $branch = (git branch 2>$null) | ForEach-Object { ([regex]'\* (.*)').Match($_) } | Where-Object { $_.Success } | ForEach-Object { $_.Groups[1].Value } | Select-Object -First 1
    }
  }
    ...
    try {
      ...
      $scriptCmd = {
        & $wrappedCmd @PSBoundParameters `
           | Where-Object { $Force -or $showDotFiles -or ($_.Name -eq $null) -or -not $_.Name.StartsWith('.') }
      }
