...
$topLevelMenuItems = Start-Process mspaint -Passthru | `
   ...
   Get-UIAControlDescendants -ControlType MenuItem,SplitButton,Custom,ListItem; #,Button;
...
foreach ($menuItemLevel1 in $topLevelMenuItems) {
  ...
  try {
    ...
    $secondLevelMenuItems = Get-UIAWindow -ProcessId $apppid | Get-UIAControlDescendants -ControlType MenuItem,SplitButton,Custom,ListItem;
